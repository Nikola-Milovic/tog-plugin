...
func TestWillThrowSmite(t *testing.T) {
	units1 := []byte("{\"name\":\"Lemi1\",\"units\":{\"archer\":[],\"knight\":[{\"x\":9,\"y\":10}]}}")
	units2 := []byte("{\"name\":\"Lemi2\",\"units\":{\"archer\":[],\"knight\":[{\"x\":9,\"y\":10}]}}")
	...
	smite["AbilityID"] = "ab_smite"
	smite["InstantCast"] = 1
	comps["AbilitiesComponent"].([]interface{})[0] = smite
